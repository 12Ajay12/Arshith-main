    ...
    <PageTransition>
      <div className="pt-32 pb-32 bg-background min-h-screen">
        <div className="container mx-auto px-4 md:px-6">
          ...
          >
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              {[
                ...
                <motion.div key={i} variants={itemVariants} className="bg-card rounded-[2.5rem] p-10 border border-border shadow-sm hover:shadow-xl transition-all group hover:-translate-y-2">
                  <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center mb-8">
                    <method.icon className="w-10 h-10 text-primary" />
                  </div>
                  <h3 className="text-3xl font-serif font-bold mb-6">{method.title}</h3>
                  <p className="text-muted-foreground leading-relaxed text-lg">{method.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="mb-40 grid md:grid-cols-2 gap-12 items-center bg-muted rounded-[3rem] p-12 md:p-20 border border-border">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 className="text-4xl md:text-5xl font-serif font-bold mb-8">Cold-Press Technology</h2>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative aspect-square">
               ...
               <div className="absolute inset-8 bg-background rounded-full flex flex-col items-center justify-center border-8 border-primary shadow-2xl z-10">
                 <span className="text-5xl font-bold text-primary mb-2">&lt; 50°C</span>
                 <span className="text-lg font-bold uppercase tracking-wider text-muted-foreground">Extraction Temp</span>
               </div>
            </motion.div>
          </div>
          ...
          >
            ...
            <div className="space-y-6">
              {[
                ...
                <motion.div key={i} variants={itemVariants} className="flex gap-8 items-start bg-card p-10 rounded-3xl border border-border overflow-hidden relative group">
                  <div className="absolute -right-4 -top-8 text-[150px] font-bold font-serif text-muted/30 select-none pointer-events-none group-hover:scale-110 transition-transform duration-700">
                    0{i+1}
                  </div>
                  <div className="relative z-10">
                    <h3 className="text-3xl font-serif font-bold mb-4 text-foreground">{item.title}</h3>
          ...
          {/* ECOSYSTEM VISION */}
          <motion.div 
            className="bg-foreground text-background rounded-[3rem] p-12 md:p-24 text-center border border-border"
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          >
            <h2 className="text-4xl md:text-6xl font-serif font-bold mb-10 text-white">The Ecosystem Vision</h2>
            <p className="text-2xl text-muted/90 max-w-4xl mx-auto leading-relaxed font-serif">
